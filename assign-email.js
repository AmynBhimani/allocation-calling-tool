// Acceptance email panel. Pick a session, preview who gets it and the exact message (with their area,
// check-in window, orientation day, and decline link), send a test, then send in batches of up to 500.
// Server does the selection, rendering, decline-token generation, and stamping (/api/assignemail).
(function () {
  ...
  function preview() {
    ...
    fetch("/api/assignemail?session=" + encodeURIComponent(session()))
  ...
  function renderPreview(d) {
    ...
    var rows = elig.map(function (e) {
      return '<tr><td>' + esc(e.name) + '</td><td>' + esc(e.area) + '</td><td>' + esc(e.region) + '</td><td>' + esc(e.orientation) + '</td><td>' + esc(e.email) + '</td></tr>';
    }).join("");
    var noRows = noem.map(function (e) {
      return '<tr><td>' + esc(e.name) + '</td><td>' + esc(e.area) + '</td><td>' + esc(e.region) + '</td></tr>';
    }).join("");

    EL("results").innerHTML =
      '<div class="card">' +
        '<h2>' + esc(d.session.name) + (d.checkIn ? '<span class="pill">Check-in: ' + esc(d.checkIn) + '</span>' : '<span class="pill" style="background:#FBEFEC;color:#7d4a41;border-color:#EAD2CE">No check-in window mapped for this session</span>') + '</h2>' +
        '<div class="kpis">' +
          '<div class="kpi go"><div class="n">' + num(c.eligible) + '</div><div class="l">Ready to email</div></div>' +
          '<div class="kpi' + (c.noEmail ? ' flag' : '') + '"><div class="n">' + num(c.noEmail) + '</div><div class="l">Accepted, no email</div></div>' +
          '<div class="kpi"><div class="n">' + num(c.alreadySent) + '</div><div class="l">Already emailed</div></div>' +
        '</div>' +
        (c.noSession ? '<div class="small">' + num(c.noSession) + ' accepted people couldn\u2019t be matched to a session by their Jamatkhana (they won\u2019t be emailed here) \u2014 worth a look.</div>' : '') +
        ...
        (elig.length ?
          ...
          '<div class="scrollx" style="margin-top:8px"><table class="matrix"><thead><tr><th>Name</th><th>Area</th><th>Region</th><th>Orientation</th><th>Email</th></tr></thead><tbody>' +
          rows + '</tbody></table></div></details>' : '') +
        (noem.length ?
          '<details><summary>' + num(noem.length) + ' accepted with no email \u2014 follow up another way</summary>' +
          '<div class="scrollx" style="margin-top:8px"><table class="matrix"><thead><tr><th>Name</th><th>Area</th><th>Region</th></tr></thead><tbody>' +
